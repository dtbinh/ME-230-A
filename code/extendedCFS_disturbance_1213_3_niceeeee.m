...
for step = 1:ss
    ...
    if obs{1}.v(2)<0 && reach == 0
        obs{1}.v = obs{1}.v + obs{1}.a*dt %+ rand*0.2*[0;1]+[0;-0.1];        
    else
        reach=1;
        obs{1}.v = [0;0.3*(-1)^step];
        slow = 0.5;
        obs{1}.a = 0;
    end
    ...
    for i=1:nstep
            ...
            %With acceleration detection
            obs{1}.v_h =obs{1}.v_h*(slow) + obs{1}.a*dt*i;
            poly = poly + obs{1}.v_h*ones(1,4)*dt;
            
            % With speed detection
            %poly = poly + obs{1}.v*ones(1,4)*dt*i;
    end
    obs{1}.v_h
   % if abs(Vobs(step))<=abs(Vobs(step+1))
    if abs(obs{1}.v_h(2))>0.4
        Will_go = Will_go* l_rate1;
        Will_stop = Will_stop* l_rate2 + 0.01;
        c_ = [c_;1];
    else
        Will_go = Will_go* l_rate2 + 0.01;
        Will_stop = Will_stop* l_rate1;
        c_ = [c_;2];
    end
    
    normalize = Will_go + Will_stop;
    
    
    Will_go_ = [Will_go_; Will_go/normalize];
    Will_stop_ = [Will_stop_; Will_stop/normalize];
    
    Will_go = Will_go/normalize;
    Will_stop = Will_stop/normalize;
    
    % Stop area calculation
    
    y_his(step) = (poly_now(2,3)+poly_now(2,2))/2;
    y_hisP = y_hisP*0.9;
    y_hisP = [y_hisP;1];
    y_hisP = y_hisP/sum(y_hisP);
    
    if  reach>0
     
        stopmode = 1;
        Y_his = y_hisP.*y_his';
        Y_std = std(y_his(end-5:end));
        if y_his(step)>y_his(step-1)
            stop_poly = [poly_now(1,:); y_his(step)+0.09+Y_std*3 y_his(step)+0.09+Y_std*3 y_his(step-1)-0.09-Y_std*3 y_his(step-1)-0.09-Y_std*3 ];
        else
            stop_poly = [poly_now(1,:); y_his(step-1)+0.09+Y_std*3 y_his(step-1)+0.09+Y_std*3 y_his(step)-0.09-Y_std*3 y_his(step)-0.09-Y_std*3 ];
        end
    else
        stopmode = 0;
    end
...
%     end
    ...
    %% Speed constraints
    Lstack = [-Vdiff(1:end-2,:) zeros(2*(nstep-1),nstep*2)];
    Sstack = ones(2*(nstep-1),1)*0.2*dt;
    Lstack = [Lstack; Vdiff(1:end-2,:) zeros(2*(nstep-1),nstep*2)];
    Sstack = [Sstack; ones(2*(nstep-1),1)*0.2*dt];
    %% Extended cost
    Mcurv = eye(nstep);
    Mcurv(nstep,nstep) = 5;
...
%     end
    ...
    if stopmode == 0
        ...
        for i=1:nstep
            for j=1:nobj
                % With acceleration detection
                obs{j}.v_h =obs{j}.v_h*(slow) + obs{j}.a*dt*i;
                poly = poly + obs{j}.v_h*ones(1,4)*dt;

                % With speed detection
                %poly = poly + obs{j}.v*ones(1,4)*dt*i;
            ...
            if i>2
                ...
                Lstack = [Lstack;zeros(1,(i-3)*dim) -lk2 -lk1 -lk0 zeros(1,(nstep-i)*dim) zeros(1,i-1) -ltheta zeros(1,nstep-i) zeros(1,nstep)];
                Sstack = [Sstack;s];
                ltheta = norm(xk0-xk1)^2;
                lk0 = 2*ur*(xk0-xk1)'-(xk1-xk2)'*P';
                lk1 = -2*ur*(xk0-xk1)'+(xk1-xk2)'*P'-(xk0-xk1)'*P;
